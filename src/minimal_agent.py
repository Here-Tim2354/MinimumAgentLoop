...
import minimal_render as render  # 终端输入和渲染。
...
def main() -> None:
    ...
    while True:
        prompt = render.read_user_message()
        if not prompt:
            break
        command = runtime.apply_slash_command(prompt)
        if command:
            kind, message = command
            (render.render_permission if kind == "permission" else render.render_think_level)(message)
            continue

        inputs.append({"role": "user", "content": prompt})

        while True:
            render.render_thinking(runtime.thinking_effort())
            ...
            for item in response.output:
                ...
                if item.type == "reasoning":
                    ...
                    if text:
                        render.render_reasoning(text)
                elif item.type == "web_search_call":
                    render.render_web_search(data.get("action"))
                elif item.type == "function_call":
                    calls.append(item)
            if not calls:
                render.render_answer(response.output_text)
            ...
            for call in calls:
                ...
                render.render_tool_call(call.name, command)
                render.render_permission(f"{runtime.permission_mode()} 权限检查中……")
                output, audit = runtime.run_tool(call.name, command, client, prompt)
                if audit:
                    render.render_permission(audit)
                render.render_tool_result(output)
